...
export const ClientPurposeSelector: React.FC<ClientPurposeSelectorProps> = ({
  ...
  const callPurposes = [
    ...
    { value: 'custom', label: '✏️ Custom Purpose', description: 'Write your own reason for calling' },
  ];
  ...
  const getFinalPurpose = () => {
    if (selectedPurpose === 'custom' && customPurpose.trim()) {
      return customPurpose.trim();
    }
    return selectedPurposeData?.label || selectedPurpose;
  };

  const handleSubmitClientInfo = () => {
    const finalPurpose = getFinalPurpose();
    const isValid = clientInfo.name.trim() &&
                    clientInfo.phone.trim() &&
                    selectedPurpose &&
                    (selectedPurpose !== 'custom' || customPurpose.trim());

    if (isValid && onClientPurposeSelected) {
      onClientPurposeSelected(clientInfo.name, clientInfo.phone, finalPurpose);
      setSubmitted(true);
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-indigo-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            ...
            We'll call you back shortly regarding: <strong>"{getFinalPurpose()}"</strong>
          </p>
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <p className="text-green-800 text-sm">
              ...
              • We already know your purpose: {getFinalPurpose()}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full p-8">
        ...
        {selectedPurpose === 'custom' && (
          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Describe your purpose for calling:
            </label>
            <textarea
              value={customPurpose}
              onChange={(e) => setCustomPurpose(e.target.value)}
              placeholder="e.g., I want to discuss a new project idea..."
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none"
              rows={3}
              maxLength={200}
            />
            <div className="text-sm text-gray-500 mt-1 text-right">
              {customPurpose.length}/200 characters
            </div>
          </div>
        )}
        ...
        {selectedPurpose === 'custom' && customPurpose.trim() && (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center space-x-2 mb-2">
              <User className="w-4 h-4 text-blue-600" />
              <span className="text-sm font-medium text-blue-800">Your Custom Purpose:</span>
            </div>
            <div className="text-blue-700 font-medium">
              {customPurpose}
            </div>
          ...
          <div className="text-center">
            ...
            {selectedPurpose && (
              <div className="text-sm text-gray-600">
                When you call, mention: <strong>"{getFinalPurpose()}"</strong>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="text-center">
            ...
            {!showClientForm ? (
              <button
                ...
                disabled={!selectedPurpose || (selectedPurpose === 'custom' && !customPurpose.trim())}
              ...
              <div className="space-y-4">
                ...
                <div className="flex space-x-3">
                  ...
                  <button
                    ...
                    disabled={!clientInfo.name.trim() || !clientInfo.phone.trim() || (selectedPurpose === 'custom' && !customPurpose.trim())}
